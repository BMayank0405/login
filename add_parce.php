...
<html>
<head>
	<title>Enlist Parcel || Evencargo</title>
	...
	?>
			...
			<div class="col-lg-7 col-md-offset-1 adparce">
				<div class="add_entry_form">
					...
					<form class="form-horizontal" role="form" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" validate autocomplete="on">
            ...
            <input type="text"  name="weight"           placeholder="weight (in gm)"   pattern="\d*" title="Number only in gm" required>
  					...
  					<select name="destination_consignment" required>
						...
						<select name="delivery_type" required>
							<option value="" disabled="disabled" selected="selected">Delivery Type</option>
							<option value="1">Cash On Delivery</option>
							<option value="0">Pre-Paid</option>
						</select>

						<input type="submit" name="enlist" value="Enlist">
					</form>
					<?php
              ...
              $weight = sanitize($_POST['weight']);
							...
							$status = 0; //inital state
							$pincode = sanitize($_POST['pincode']);
							$deliver_type = sanitize($_POST['delivery_type']);
              $charge_array = charges_calculate($weight,$cod_amount,$deliver_type);
              ...
              if($deliver_type == 1){
                ...
                VALUES(null,'$client','$awb_no',$pincode,'$customer_name',$customer_phone,
  						...
  						$c_result = $db->query($c_query);
							...
							if($c_result)
							{
								 echo "<div class='btn btn-success'>Item enlisted with awb number : $awb_no </div>";

							}
							else
							{
								echo "<div class='btn btn-error'>Error Enlisting</div>";
							}
						}
                ...
                $i=[];
                  ...
                  if(in_array('',$filesop)){
                    ...
                    $pincode = sanitize($filesop[2]);
  									...
  									$deliver_type = trim(strtoupper(sanitize($filesop[9])));
                    if($deliver_type === 'COD'){
                      $deliver_type = 1;
                    }
                    else {
                    ...
                    $weightcharge = $charge_array['weightcharge'];
  									$status = 0; //inital state
                    $today = date("Y-m-d");
                    $origin = trim(strtolower(sanitize($filesop[11])));
                    $destination = trim(strtolower(sanitize($filesop[12])));

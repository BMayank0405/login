        ...
        <div class="add_entry_form">
          ...
          <?php
            if(isset($_REQUEST['delivered_items']))
            {
              ...
              foreach($id as $val)
              {
                $query = "UPDATE `customer` SET `status` = 3 WHERE `id` = $val";
                $result = $db->query($query);
              }
              if($result)
              {
                echo "<div class='btn btn-success'>Not Delivered</div>";
              }
              else
              {
             ...
             if(mysqli_num_rows($result) > 0)
             {
              ...
              while($rows = mysqli_fetch_assoc($result))
              {
                ...
                $counter+=1;
                            ...
                            echo "</table><input type='submit' name='delivered_items' value='Failed Delivery'>";
             }

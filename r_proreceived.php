  ...
  <div class="alpha_inside">
      ...
      <div class="col-lg-7 form_entry_col">
        <div class="add_entry_form">
          ...
          <table class="table table-striped report-table" id="_report_table"> <tr> <td>S.No</td> <td>Id</td> <td>Customer</td> <td>Address</td>  <td>Price</td> <td>Received  <input type="checkbox" name="select_all" id="select_all"></td> </tr>
          <?php
             ...
             if(mysqli_num_rows($result) > 0)
             {
              ...
              while($rows = mysqli_fetch_assoc($result))
              {
                ...
                $counter+=1;
                            ...
                            echo "</table><input type='submit' name='r_proreceived' value='Received Items'>";
             }
          ...
          <?php
            if(isset($_REQUEST['r_proreceived']))
            {
              $r_id = $_REQUEST['r_id'];
              $today = date("Y-m-d");
              foreach($r_id as $val)
              {
                  $query = "UPDATE `return_product` SET `status` = 22, `return_date` = '$today' WHERE `r_id` = $val";
                  $result = $db->query($query);
              }
              if($result)
              {
                echo "<div class='btn btn-success'>Product Received</div>";
              }
              else
              {

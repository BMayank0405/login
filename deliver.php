...
<html>
<head>
  <title>Product Deliver || Evencargo</title>
...
<div class="container-fluid staff_panel_ind c_container">
  ...
  <div class="alpha_inside">
    ...
    <?php
            if(isset($_REQUEST['delivered_items']))
            {
              $id = $_REQUEST['id'];
              $today = date("Y-m-d");
              foreach($id as $val)
              {   $query = "UPDATE `customer` SET `status` = 2, `dispatch_date` = '$today' WHERE `id` = $val";
          ...
          <table class="table table-striped report-table" id="_report_table"> <tr> <td>S.No</td> <td>Client_order_id</td><td>Customer</td> <td>Address</td>  <td>Price</td> <td>Delivered  <input type="checkbox" name="select_all" id="select_all"></td></tr>
          <?php
             ...
             if(mysqli_num_rows($result) > 0)
             {
              ...
              while($rows = mysqli_fetch_assoc($result))
              {
                ...
                $counter+=1;
                            ...
                            echo "</table><input type='submit' name='delivered_items' value='Delivered Items'>";
             }

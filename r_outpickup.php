  ...
  <div class="alpha_inside">
      ...
      <div class="col-lg-7 form_entry_col">
        <div class="add_entry_form">
          ...
          <?php
            if(isset($_REQUEST['r_outpickup']))
            {
              $r_id = $_REQUEST['r_id'];
              $today = date("Y-m-d");
              foreach($r_id as $val)
              {
                  $query = "UPDATE `return_product` SET `status` = 21, `request_date` = '$today' WHERE `r_id` = $val";
                  $result = $db->query($query);
              }
              if($result)
              {
                echo "<div class='btn btn-success'>Requested Pickup</div>";
              }
              else
              {
          ...
          <table class="table table-striped report-table" id="_report_table"> <tr> <td>S.No</td> <td>Id</td> <td>Customer</td> <td>Address</td> <td>Price</td> <td>Out For Pickup  <input type="checkbox" name="select_all" id="select_all"></td> </tr>
          <?php
             ...
             if(mysqli_num_rows($result))
             {
              ...
              while($rows = mysqli_fetch_assoc($result))
              {
                ...
                $counter+=1;
                            ...
                            echo "</table><input type='submit' name='r_outpickup' value='Out for pickup'>";
             }
